...
- Bring your own storage: CoreWeave CAIOS, S3-compatible storage, Google Cloud
  Storage, Cloudflare R2, or memory for tests.
- Bring your own compute: CoreWeave Sandboxes, Modal, Kubernetes, Docker, E2B,
  Cloudflare Containers, local subprocesses, or no kernels.
- Bring your own identity: OpenID Connect, Cloudflare Access, or a local dev
  bypass.
- Optional managed AI and project secrets, with server-side keys instead of
  notebook-supplied credentials.
- A generated `MARIMOHUB_*` configuration reference and an OpenAPI-backed
  TypeScript client.
...
Requirements: Node >= 22.12, pnpm 10.20.0, and `uv` plus Python if you want to
