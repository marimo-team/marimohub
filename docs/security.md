...
- OIDC requires `MARIMOHUB_AUTH_ALLOWED_EMAIL_DOMAINS` (verified email). marimohub
  is an internal tool, so it will not admit every account the IdP can
  authenticate; set explicit domains, or `*` to consciously allow all.
