...
| ------------- | ------------------------- |
| **Status**    | Draft                     |
| **Author**    | Myles Scolnick            |
| **Created**   | 2026-06-17                |
| **Reviewers** | @Michael, @Dylan, @Akshay |
...
This RFC proposes shipping that v1 and lays out the rationale, the rejected alternatives, the accepted risks, and the modules deliberately left for later (notably **AI** and **secrets**, §10).
...
2. **Read-side tenant isolation.** Every authenticated user can _see_ the whole catalog; writes are role-gated. Per-caller catalog filtering is deferred ([`bucket_spec.md` §12, §15](./bucket_spec.md)).
...
4. **Realtime multi-user editing of a notebook.** Collaborative editing is the kernel/marimo runtime's concern, not the hub's. The hub tracks _sessions_, not live document state.
5. **Managed secrets and AI governance.** Deferred to dedicated modules (§10).
...
| Port | Interface (abridged) | Reference adapter | Other adapters |
| --- | --- | --- | --- |
| **Storage** | `Bucket` — `get/head/put/delete/list`, conditional `put` (CAS on ETag) | S3-compatible — CoreWeave CAOIS (also AWS S3, MinIO) | Cloudflare R2 (binding); `memory` (tests) |
| **Compute** | `SandboxProvider.create(id)` → `SandboxInstance`; `proxy(request)`; optional `listActive()` (reconciliation) | CoreWeave (sandboxes on CKS) | Modal; Cloudflare Containers (DO); `none` (browse-only) |
| **AuthN** | `Authenticator.authenticate(request)` → `AuthUser \| null` | OIDC | Cloudflare Access; `dev` bypass |
| **AuthZ** | `Authorizer.can(user, action, resource)` — reads roles from `Bucket` | roles in storage (no extra tech) | same |
...
**Why:** It is the only structure that satisfies "bring your own storage and compute" and "deploy anywhere" simultaneously. Because the core never knows its provider, the same code runs as the reference deployment (CoreWeave — CAOIS storage, CoreWeave sandboxes, OIDC) and, unmodified, on a plain Docker host (S3 + Modal) or Cloudflare (R2 + Containers + Access).
...
### 6.7 Reference deployment: a Docker app on CoreWeave

**Decision:** The reference, documented, first-class deployment is the **Node app packaged as a Docker container and configured by `MARIMOHUB_*` environment variables**, run on **CoreWeave Kubernetes (CKS)** with **CAOIS** (CoreWeave's S3-compatible object storage), **CoreWeave sandboxes** for compute, and **OIDC** for auth. A plain Docker host and Cloudflare (Workers + R2 + Containers + Access) are fully-supported alternatives, selected by configuration alone.

**Why:** Docker + env config is the most portable packaging — it runs anywhere Docker runs and is the config-driven common case (§6.6). CoreWeave supplies the team's own compute and storage: CAOIS is S3-compatible, so it plugs into the standard storage adapter with no new code, and kernels run in CoreWeave sandboxes on CKS. Because everything sits behind the ports, the same image redeploys on a plain Docker host or on Cloudflare by swapping configuration — CoreWeave is the reference target, not a dependency.
...
| --- | --- | --- | --- |
| 1 | **Conditional-write dependency.** The catalog CAS requires _atomic_ `If-Match`; not all S3-compatible stores support it well, and a non-atomic check-then-set corrupts the catalog only under real contention. | Medium | Blessed backends with atomic conditional writes (S3, R2, CAOIS, MinIO); boot self-check refuses a non-compliant store (§5.2). Other S3-compatible stores: §12.3 ([`bucket_spec.md` §1](./bucket_spec.md)). |
...
| 8 | **`POST` creates are not idempotent.** A retry can duplicate a project/notebook/session. | Low | Accepted in v1; duplicates are cheap and user-deletable. Optional `Idempotency-Key` later ([`bucket_spec.md` §13](./bucket_spec.md)). |
...
- **Write authorization.** Writes are role-gated against the target project (`viewer` < `editor` < `admin`; owner is implicitly admin), enforced server-side per route, never in the client ([`bucket_spec.md` §12](./bucket_spec.md)). Insufficient role → `403`.
...
- **Secrets in notebooks.** Without a secrets module (§10.2), users put secrets directly in notebook code — a real risk, and a primary motivation for that module.
...
- **Read-side tenant isolation / true multi-tenancy.** Filtering the catalog per-caller changes the read model (membership check per project; the "2 GETs" guarantee no longer holds). Scope before opening to untrusted users. ([`bucket_spec.md` §12, §15](./bucket_spec.md))
...
## 10. Future Modules (not yet designed)

Two modules are known missing — flagged here to be tracked, not designed. Each should follow the same ports-and-adapters discipline (a `core` interface, swappable adapters) when picked up.

### 10.1 AI module

AI is out of scope as a hub-level concern for v1 — no managed model credentials, no proxy/metering, no hub-side features. A module would most likely hand model credentials to sandboxes (so users don't hardcode keys — overlaps with §10.2) and optionally proxy/meter LLM calls through the hub.

### 10.2 Secrets module

Out of scope for v1. The Sandbox port can inject env vars (`setEnvVars`), but there is no hub concept for **storing, scoping, or managing** secrets above that — and under no-DB they must live somewhere deliberate: an external secrets manager (Vault, AWS/Cloudflare) or encrypted-in-bucket (which needs a key-management story). Without it, users would paste keys into notebook code (§8).

---

## 11. Rollout and Milestones

1. Deploy to CoreWeave Kubernetes (CKS) backed by CoreWeave Object Storage (CAOIS) (2 weeks)
2. Open-source quietly (marimo-team/marimo-hub), and deploy to an enterprise design-partner (2 weeks)
3. General Availability and evangelize (2 weeks)
4. Fast feedback and iterate (2 weeks)

---

## 12. Appendix: Condensed Architecture & Storage Spec

> A self-contained digest of `architecture.md` and `bucket_spec.md`, which are not part of this presentation. §5 gives the high-level shape; this carries the detail needed to read the RFC standalone. (Both source docs are ~10× longer.)
...
```
1 SPA      GET /api/projects/{pid}/notebooks/{nid}
...
projects/{pid}/
  ...
  notebooks/{nid}/
    ...
    README.md
    notebook/                  ← impl namespace, never exposed via the API
      source.json              ← typed source pointer (v1: local)
      notebook.py · pyproject.toml          ← latest code
...
**Storage requirement** — strong read-after-write consistency + _atomic_ conditional writes (`If-Match` / `If-None-Match`); blessed backends and the boot check are in §5.2, other S3-compatible stores in §12.3.
...
### 12.3 Other S3-compatible storage
...
- [`architecture.md`](./architecture.md) and [`bucket_spec.md`](./bucket_spec.md) — the source design docs, **condensed in §12** (not part of this presentation).
