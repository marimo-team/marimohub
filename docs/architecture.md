...
_adapters_. Cloudflare (Workers + R2 + Containers + Access) is the reference
deployment, but every component can be swapped without touching the rest of the
system.

> **The one idea to take away:** MarimoHub is not "a Cloudflare app." It is a
> provider-agnostic core with Cloudflare adapters. Swap the storage adapter and
> it runs on MinIO; swap the compute adapter and it runs on Modal; swap the auth
> adapter and it runs behind any OIDC provider. Nothing in the domain logic
...
```
        ...
        └──────────────────────────│───────────────│────────│────────┘
                  ...
                  reads roles from │               │        │ provisions
                       ...
                       │ R2 (binding)       │  │ Modal                │
                       │ memory (for tests) │  │ local (host subproc) │
                       └────────────────────┘  └─────────────────────┘
```
...
| **Adapters** | **S3-compatible** (AWS S3, Cloudflare R2, MinIO, Tigris, Ceph) · **memory** (tests / local dev)                                                              |
| **Schema**   | Iceberg-inspired: a single mutable `catalog.json` pointer → immutable snapshots → per-notebook folders. Full detail in [`bucket_spec.md`](./bucket_spec.md). |
...
mutable file; everything else is immutable or append-only. This gives consistent
reads, safe concurrent writes, and free audit history without a transactional
database. The single hard requirement on an adapter is **strong
...
- _Where do a notebook's source bytes live?_ — Declared per-notebook in
  ...
  - `local` — bytes stored in the object store (`notebook/notebook.py`).
...
| **Adapters**      | **Cloudflare Containers** (Durable Object-backed, the reference) · **Modal** · **local** (host subprocess via `uv run marimo edit`, dev only)                                                                                     |
...
teardown:  copy files back (if mounted-fallback) → destroy sandbox
```
...
- **Served at** `/api/doc` (machine-readable spec); routes under `/api/*`.
...
| --------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Auth      | `GET /api/me`                                                                                                                |
| Projects  | `GET/POST /api/projects` · `GET/PUT/DELETE /api/projects/{pid}`                                                              |
| Notebooks | `GET/POST /api/projects/{pid}/notebooks` · `GET/PUT/DELETE …/{nid}` · `GET …/{nid}/content` · `GET …/{nid}/versions[/{vid}]` |
| Sessions  | `POST …/{nid}/sessions` (provision a sandbox) · `POST …/{sid}/heartbeat` · `DELETE …/{sid}`                                  |
| Sandbox   | `POST /api/sandbox` · `POST /api/sandbox/{id}/exec` · `DELETE /api/sandbox/{id}`                                             |
...
| Variable                                 | Purpose                                  |
| ---------------------------------------- | ---------------------------------------- |
| `MARIMOHUB_STORAGE_BACKEND`              | `s3` \| `r2` \| `memory`                 |
...
| Variable                                       | Purpose                                                        |
| ---------------------------------------------- | -------------------------------------------------------------- |
| `MARIMOHUB_COMPUTE_BACKEND`                    | `cloudflare` \| `modal` \| `local` \| `none`                   |
...
| `MARIMOHUB_AUTH_ACCESS_TEAM`            | Cloudflare Access team (for `cloudflare-access`) |
| `MARIMOHUB_AUTH_ACCESS_AUD`             | Cloudflare Access application audience           |
...
_(The proof-of-concept in this repo uses a flatter set of vars — `AUTH_MODE`,
`USER_ID`, `R2_BUCKET_NAME`, `R2_S3_ENDPOINT`, `SANDBOX_HOSTNAME`, … — that this
prefixed scheme supersedes.)_
...
| **Storage**    | R2 (binding)                                        | S3 / MinIO                        | S3 / MinIO / Ceph            |
| **Compute**    | Cloudflare Containers (DO)                          | Modal (or `local` for dev)        | Modal, or in-cluster pods    |
