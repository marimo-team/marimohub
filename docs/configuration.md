...
| `MARIMOHUB_BIND_HOST` | IP address or hostname the HTTP server listens on. Unset listens on all interfaces, which a container needs to publish its port. Set `127.0.0.1` to accept only local connections, such as behind a reverse proxy on the same host. | — | — | `127.0.0.1` |
