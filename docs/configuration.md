...
| `MARIMOHUB_VIEWER_MODE` | What a user whose effective role is `viewer` sees when opening a notebook (static \| ephemeral-sandbox). `static` serves the last captured HTML snapshot (no compute, no code execution); `ephemeral-sandbox` provisions a real kernel whose edits are discarded on teardown (no version, snapshot, or workspace write-back). Applies to any effective viewer — via `MARIMOHUB_DEFAULT_ROLE=viewer` or an explicit viewer membership. Editors and above are unaffected. See [Auth -> What viewers see](./auth.md#what-viewers-see-marimohub_viewer_mode). | — | `static` | `ephemeral-sandbox` |
