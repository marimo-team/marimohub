...
| `MARIMOHUB_COMPUTE_KUBERNETES_RUN_AS_USER` | `runAsUser` for the kernel Pod `securityContext`, with `runAsNonRoot` (unless `0`) and a matching `fsGroup`. Set when the cluster admission policy rejects Pods that do not pin a uid. Must be a user that can write the image workdir. Unset leaves the securityContext off. | — | — | `1000` |
