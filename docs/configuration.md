...
| `MARIMOHUB_COMPUTE_KUBERNETES_HOSTNAME_TEMPLATE` | Kernel URL template. Supports `{id}`, `{name}`, `{namespace}`, `{port}`, `{host}`, and `{token}`. Proxy exposure defaults to the internal Service URL. Set this only for a different cluster DNS domain. | — | `subdomain https://{id}.{host}, proxy http://{name}.{namespace}.svc.cluster.local:{port}` | — |
