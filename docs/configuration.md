...
| `MARIMOHUB_AUTHZ_SUBJECT_CONTEXT_BACKEND` | Set `library` to load a trusted subject-context provider. The provider resolves clearance and compartments for each principal. A classification order is required. Without a provider, all labeled resources are denied. | — | — | `library` |
