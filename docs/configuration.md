...
| `MARIMOHUB_REMOTE_DEVELOPMENT` | Brokered remote-development mode (`disabled` or `ssh`). SSH requires exclusive editor sandboxes, a compatible compute backend, and explicitly marked images. | — | `disabled` | `ssh` |
| `MARIMOHUB_REMOTE_DEVELOPMENT_IMAGES` | Comma-separated exact references from MARIMOHUB_COMPUTE_IMAGE that implement the remote-development image contract. | — | — | `ghcr.io/orgname/marimo-sandbox:remote-dev` |
