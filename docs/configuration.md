...
| `MARIMOHUB_EDITOR_SANDBOX_SHARING` | Whether project editors share one persistent notebook sandbox (`shared`) or use exclusive ownership (`exclusive`). In exclusive mode, other editors can use a temporary sandbox or request a warned takeover. This setting does not affect apps or viewer sessions. See [Editor sessions](./editor-sessions.md). | — | `shared` | `exclusive` |
