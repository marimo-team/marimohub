...
| Variable | Description | Required | Default | Example |
| --- | --- | --- | --- | --- |
| `MARIMOHUB_COMPUTE_IMAGE` | Container image with marimo + uv + python. Required by the `modal` backend; recommended for `coreweave`. | — | — | `ghcr.io/orgname/marimo-sandbox:latest` |
...
| `MARIMOHUB_COMPUTE_ASSET_URL` | Base URL for marimo frontend assets (e.g. a CDN). Omit to use the bundled assets. | — | — | `https://cdn.jsdelivr.net/npm/@marimo-team/...` |
...
| `MARIMOHUB_COMPUTE_E2B_TEMPLATE` | E2B template id with marimo + uv + python. Falls back to `MARIMOHUB_COMPUTE_IMAGE`. | — | — | `marimo` |
...
| `MARIMOHUB_AUTH_ALLOWED_EMAIL_DOMAINS` | Comma-separated email-domain allowlist (requires a verified email). Required so an internal deployment cannot silently admit every account the IdP authenticates; set `*` to explicitly allow all domains. A single domain is also sent to Google as the `hd` hint. | Yes | — | `marimo.io,example.com` |
...
| `MARIMOHUB_MAX_SESSIONS_PER_USER` | Per-user concurrent session cap (`0` = unlimited). | — | `10` | — |
...
| `MARIMOHUB_VIEWER_MODE` | What a user whose effective role is `viewer` sees when opening a notebook (static \| ephemeral-sandbox). `static` serves the last captured HTML snapshot (no compute, no code execution); `ephemeral-sandbox` provisions a real kernel whose edits are discarded on teardown (no version, snapshot, or workspace write-back). Applies to any effective viewer — via `MARIMOHUB_DEFAULT_ROLE=viewer` or an explicit viewer membership. Editors and above are unaffected. | — | `static` | `ephemeral-sandbox` |
...
| Variable | Description | Required | Default | Example |
| --- | --- | --- | --- | --- |
| `MARIMOHUB_WIF_BROKER` | Which credential broker exchanges the JWT for temporary creds. Required when WIF is enabled (no default, so the federated cloud is always explicit). Currently `coreweave`; add an adapter implementing the core CredentialBroker port for more. | — | — | `coreweave` |
