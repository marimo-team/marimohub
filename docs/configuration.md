...
| `MARIMOHUB_COMPUTE_COREWEAVE_RUNNER_IDS` | Comma-separated runner id(s) sandboxes may schedule on (own-cluster/CKS runners); the runner's default policy decides network, mounts, and placement. Omit to let the fleet schedule anywhere. | — | — | `runner-marimohub` |
