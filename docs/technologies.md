...
interfaces (ports), this maps each one to the _technology_ that implements it in
the reference **Cloudflare** deployment. Swapping a provider means swapping the
library noted here — the ports stay fixed.

## Port → reference technology

| Component (port)                 | Reference technology                          | Off-Cloudflare alternative         |
| -------------------------------- | --------------------------------------------- | ---------------------------------- |
...
## Runtime & Hosting (reference deployment)

### Cloudflare Workers

Serverless JavaScript/TypeScript runtime at the edge. Hosts the Hono API
(`@marimo-hub/api`) and holds the R2 and Durable Object bindings. The reference
...
The reference sandbox image (`examples/cloudflare-worker/sandbox.Dockerfile`) is
...
context (`Hono<{ Bindings: Env }>`). Auth and catalog-init run as `/api/*`
...
**OpenAPI 3.1 document** (served at `/api/doc`). The spec therefore cannot drift
...
Generates **ULID**-based IDs (`proj_`, `nb_`, `snap_`, `ver_`, `sess_`) that sort
chronologically. A `monotonicFactory` is used for event keys so events written in
the same millisecond still sort in append order. (Ephemeral sandbox IDs use a
short `crypto.randomUUID()` slice instead.)
