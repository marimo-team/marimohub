...
`envFrom: secretRef`. A secrets manager (External Secrets Operator, Vault,
Doppler, …) can sync into that Secret; otherwise manage it directly. To change a
value, update the Secret and restart.
...
1. Check Deployment readiness and `/healthz`.
2. Sign in through OIDC.
3. Create and save a notebook.
4. Start a CoreWeave Sandbox kernel.
5. Restart an API pod and confirm the notebook remains available.

## Production cautions

- Keep the API Deployment stateless.
- Run maintenance as exactly one replica.
- Store CAIOS, CoreWeave, OIDC, and session secrets in a Secret or synced secret
  manager.
- Use a separate sandbox hostname for kernel traffic.

## Troubleshooting

See [Troubleshooting](../troubleshooting.md), especially login failures and
kernel reachability.
