...
- **Authorization.** Writes are role-gated against the target project (`viewer` < `editor` < `manager` < `admin`; owner is implicitly admin), enforced server-side per route, never in the client ([`bucket_spec.md` §12](./bucket_spec.md)). Manager is the highest assignable role; admin is reserved for owners, deployment super admins, and legacy member rows. Insufficient role → `403`. Reads are gated at `viewer`: open when `MARIMOHUB_DEFAULT_ROLE` is set (the default), membership-gated (non-members get `404`) when it is `none`.
