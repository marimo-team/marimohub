...
| ----------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------------- |
| **Storage** | `Bucket` — `get/head/put/delete/list`, conditional `put` (CAS on ETag)                                       | S3-compatible — CoreWeave CAIOS (also AWS S3, MinIO) | Cloudflare R2 (binding); `memory` (tests)               |
...
| Storage    | CAIOS (S3-compatible)       | S3 / MinIO           | R2 binding                            |
...
**Why:** It is the only structure that satisfies "bring your own storage and compute" and "deploy anywhere" simultaneously. Because the core never knows its provider, the same code runs as the reference deployment (CoreWeave — CAIOS storage, CoreWeave sandboxes, OIDC) and, unmodified, on a plain Docker host (S3 + Modal) or Cloudflare (R2 + Containers + Access).
...
**The common case (config-driven).** A standard deployment is just environment variables on the stock Docker image — no code. `createFromEnv()` reads each `*_BACKEND` selector, builds the matching adapter, and hands the wired `ApiDeps` to `createApi`. The reference shape (S3 storage + Modal compute + OIDC) is:
...
| 8   | **`POST` creates are not idempotent.** A retry can duplicate a project/notebook/session.                                                                                                                                                                                                                  | Handled                    | Optional `Idempotency-Key` on the create routes replays the stored result on retry ([`idempotency.md`](./idempotency.md)); sessions also de-dupe on `(user, notebook)`.                                    |
...
1. Deploy to CoreWeave Kubernetes (CKS) backed by CoreWeave Object Storage (CAIOS) (2 weeks)
