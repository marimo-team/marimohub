...
| **Adapters**      | **Cloudflare Containers** (Durable Object-backed, the reference) · **Modal** · **CoreWeave** (CoreWeave Sandboxes via the vendored `@coreweave/cwsandbox` SDK, Node/gRPC) · **Kubernetes** (a Pod + Service + Ingress per session via the cluster API) · **Docker** (a container per kernel on a local or remote Docker engine) · **E2B** (E2B sandboxes; bring-your-own `e2b` SDK) · **local** (host subprocess via `uv run marimo edit`, dev only) |
...
> **Cloudflare Worker example** — the `examples/cloudflare-worker` reference
> deployment reads a flatter set of vars from the Workers runtime binding
...
packages/
  ...
  compute-docker/         Docker engine adapter (a container per kernel)
