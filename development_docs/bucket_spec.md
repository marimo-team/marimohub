...
| `projects/{pid}/notebooks/{nid}/_session_commit_lock.json`     | JSON     | Ten-minute CAS lease owned by `NotebookService`. Session commit and delete hold it while changing notebook objects. An expired lease can be replaced so a crashed holder cannot block the notebook permanently. The lease uses process wall-clock time; replicas must have reasonably synchronized clocks, and a protected operation must finish within the lease to preserve mutual exclusion. Legacy records without `expires_at` expire ten minutes after their bucket upload time.                 |
