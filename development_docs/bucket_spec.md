    ...
    └── {project-id}/
        ...
        └── notebooks/
            └── {notebook-id}/
                ...
                ├── _session_commit_lock.json ← content-write/delete CAS lease
...
| `projects/{pid}/notebooks/{nid}/_session_commit_lock.json`       | JSON     | Ten-minute CAS lease owned by `NotebookService`. Local session commits, Git syncs, and deletion hold and renew it. Expired leases are replaceable, so a crashed holder cannot permanently block the notebook. Replicas need reasonably synchronized clocks; a storage outage lasting through the lease can still prevent renewal. Legacy records without `expires_at` expire ten minutes after upload.                                                                                                 |
