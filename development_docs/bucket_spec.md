    ...
    └── {project-id}/
        ...
        └── notebooks/
            └── {notebook-id}/
                ...
                ├── _session_commit_lock.json ← session-commit/delete CAS lock
...
| `projects/{pid}/notebooks/{nid}/_session_commit_lock.json`     | JSON     | CAS lock owned by `NotebookService`. Session commit and delete hold it while changing notebook objects, so their multi-object writes cannot overlap. The lock has no lease: if a process dies while holding it, later operations fail with a conflict until an operator resets the lock or hard deletion removes it.                                                                                                                                                                                   |
