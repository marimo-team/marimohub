...
│   ├── app-pools/
│   │   └── {project-id}/
│   │       └── {notebook-id}.json           ← reservations, assignments, and visit leases (CAS)
│   ├── apps/
│   │   └── {project-id}/
│   │       └── {notebook-id}.json           ← legacy app-session claim (CAS)
│   ├── editors/
│   │   └── {project-id}/
...
| `_system/app-pools/{pid}/{nid}.json`                                      | JSON     | Versioned pool record owned by `AppPoolStore`: reservations, account assignments, visit leases, and deletion tombstones. See [App pool records](#app-pool-records).                                                                                                                                                                                                                                                                                                                                    |
| `_system/apps/{pid}/{nid}.json`                                           | JSON     | Legacy app-singleton claim, released through `SessionService` during pool migration. New app admissions use `_system/app-pools/{pid}/{nid}.json`. See §4.8.1.                                                                                                                                                                                                                                                                                                                                          |
...
**App sessions.** A session record may carry `mode: "app"`: the notebook served as a read-only application via `marimo run` instead of the editor. [App pool records](#app-pool-records) route accounts to versioned sandboxes; `user_id` records the starter for aggregate compute caps. Apps load copy-only and never write back notebook code, outputs, workspace files, or filesystem snapshots. `app_pool: true` identifies managed pool members; `source_version_id` pins their committed source. `active_connections` and `connections_checked_at` hold the last kernel connection probe. Absent `mode` means `edit`.

### 4.8.1 `_system/apps/{pid}/{nid}.json`

This legacy singleton claim remains readable during migration. New app admissions use [app pool records](#app-pool-records) and never acquire this claim.

```json
{ "session_id": "sess-4m8v6k2q9r5t7w3x", "claimed_at": "2025-03-05T14:30:00Z" }
```

`SessionService.releaseApp` CAS-writes the free marker `session_id: null` only when the claim still names the retiring session. A raw delete could remove a different holder acquired after the read. Notebook or project deletion may remove subordinate claims as cleanup.

Legacy apps are adopted as drain-only pool members. Stop singleton writers before enabling pool writers; see [the coordinated rollout requirements](../docs/app-pools.md#upgrade).
