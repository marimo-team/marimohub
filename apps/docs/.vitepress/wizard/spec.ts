...
export const AUTH_WIRING: Record<string, BackendWiring> = {
	...
	'proxy-header': {
		...
		rhs: () =>
			[
				`(() => {`,
				`\tconst domainsValue = process.env.MARIMOHUB_AUTH_ALLOWED_EMAIL_DOMAINS?.trim();`,
				`\tif (!domainsValue) {`,
				...
				`\t\tthrow new Error('MARIMOHUB_AUTH_ALLOWED_EMAIL_DOMAINS lists no domains.');`,
				`\t}`,
				``,
				`\tconst jwtConfigured = [`,
				`\t\tprocess.env.MARIMOHUB_AUTH_PROXY_JWT_ISSUER,`,
				`\t\tprocess.env.MARIMOHUB_AUTH_PROXY_JWT_AUDIENCE,`,
				`\t\tprocess.env.MARIMOHUB_AUTH_PROXY_JWKS_URL,`,
				`\t].some((value) => Boolean(value?.trim()));`,
				`\tconst parseHeaders = () => {`,
				`\t\tconst raw = process.env.MARIMOHUB_AUTH_PROXY_HEADER;`,
				`\t\tif (raw === undefined || raw.trim() === '') return undefined;`,
				`\t\tconst headers = raw.split(',').map((header) => header.trim());`,
				`\t\tconst maximum = jwtConfigured ? 1 : 2;`,
				`\t\tconst headerName = new RegExp(${q("^[!#$%&'*+\\-.^_`|~0-9A-Za-z]+$")});`,
				`\t\tif (headers.length > maximum || headers.some((header) => !headerName.test(header))) {`,
				`\t\t\tthrow new Error(`,
				`\t\t\t\tjwtConfigured`,
				...
				``,
				`\tif (jwtConfigured) {`,
				`\t\tconst audience = process.env.MARIMOHUB_AUTH_PROXY_JWT_AUDIENCE?.trim();`,
				`\t\tif (!audience) {`,
				`\t\t\tthrow new Error(`,
				...
				`\t\tconst issuer = process.env.MARIMOHUB_AUTH_PROXY_JWT_ISSUER?.trim() || undefined;`,
				`\t\tconst jwksValue = process.env.MARIMOHUB_AUTH_PROXY_JWKS_URL?.trim();`,
				`\t\tconst jwksUrl = (() => {`,
				`\t\t\tif (!jwksValue) return undefined;`,
				`\t\t\ttry {`,
				`\t\t\t\tconst url = new URL(jwksValue);`,
				`\t\t\t\tif (url.protocol === 'https:' && !url.username && !url.password) {`,
				`\t\t\t\t\treturn url.toString();`,
				`\t\t\t\t}`,
				`\t\t\t} catch {}`,
				`\t\t\tthrow new Error(`,
				`\t\t\t\t'MARIMOHUB_AUTH_PROXY_JWKS_URL must be an HTTPS URL without credentials.',`,
				`\t\t\t);`,
				`\t\t})();`,
				`\t\treturn new ProxyHeaderAuthenticator({`,
				`\t\t\tmode: 'jwt',`,
				`\t\t\taudience,`,
				`\t\t\tallowedEmailDomains,`,
				...
				`\tconst headers = parseHeaders();`,
				`\tif (!headers) {`,
				`\t\treturn new ProxyHeaderAuthenticator({ mode: 'headers', allowedEmailDomains });`,
				`\t}`,
				`\tif (headers.length === 1) {`,
				`\t\treturn new ProxyHeaderAuthenticator({`,
				`\t\t\tmode: 'headers',`,
