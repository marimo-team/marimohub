...
import type { CommandProcess, ProcessResult, SandboxInfo } from '@coreweave/cwsandbox';
...
interface FakeSandbox {
	...
	runCalls: string[][];
	startCalls: string[][];
	writes: { path: string; content: unknown }[];
...
function makeWorld(opts?: { runImpl?: (cmd: readonly string[]) => Promise<ProcessResult> }) {
	...
	function reconnect(entry: { fake: FakeSandbox }, cwId: string) {
		return {
			...
			files: {
				...
				write: async (path: string, content: unknown) => {
					entry.fake.writes.push({ path, content });
				},
			},
			delete: async () => {
				entry.fake.deleted++;
	...
	return { client, created, deleted, registry };
}
...
describe('CoreWeaveCompute', () => {
	...
	describe('re-resolved instance', () => {
		it('reconnects to the existing sandbox by tag instead of creating a new one', async () => {
			...
			await compute.create(SANDBOX_ID).writeFile('/workspace/x', 'hi');
			expect(world.created).toHaveLength(1); // no second create
			expect(world.registry.get('cw-1')!.fake.writes.at(-1)).toMatchObject({
				path: '/workspace/x',
				content: 'hi',
			});
		});
	...
	describe('readFile() / writeFile()', () => {
		...
		it('writeFile forwards path and content to the sandbox', async () => {
			const world = makeWorld();
			const inst = makeCompute(world).create(SANDBOX_ID);
			await inst.writeFile('/workspace/out.py', 'data');
			expect([...world.registry.values()][0].fake.writes.at(-1)).toMatchObject({
				path: '/workspace/out.py',
				content: 'data',
			});
		});

		it('writeFile creates the parent directory first (nested path)', async () => {
			const world = makeWorld();
			const inst = makeCompute(world).create(SANDBOX_ID);
			await inst.writeFile('/tmp/marimohub-config/marimo/marimo.toml', 'x');
