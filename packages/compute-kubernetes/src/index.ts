 ...
 *    main process.
 *  - `exec`/`readFile`/`writeFile`/`listFiles` all go through the Pod `exec`
...
class KubernetesSandboxInstance implements SandboxInstance {
	...
	async writeFile(path: string, content: string): Promise<void> {
		...
		const res = await this.client.exec(
			this.name,
			['sh', '-lc', `mkdir -p ${shellQuote(dir)} && cat > ${shellQuote(path)}`],
			content,
		);
		if (res.exitCode !== 0) throw new Error(`writeFile ${path} failed: ${res.stderr}`);
	}
	...
	async listFiles(path: string, options?: ListFilesOptions): Promise<ListFilesResult> {
		...
		try {
			...
			const res = await this.exec(
				`find ${shellQuote(path)} -mindepth 1 ${depth} -printf '%y\\t%s\\t%p\\n'`,
			);
			if (!res.success) return { success: false, files: [] };
			const files: FileInfo[] = [];
			for (const line of res.stdout.split('\n')) {
				if (!line) continue;
				const [typeChar, sizeStr, absolutePath] = line.split('\t');
				...
				files.push({
					name,
					absolutePath,
					relativePath: absolutePath.startsWith(path)
						? absolutePath.slice(path.length).replace(/^\//, '')
						: absolutePath,
					type:
						typeChar === 'f'
							? 'file'
...
export class KubernetesCompute implements SandboxProvider {
	...
	create(id: SandboxId): SandboxInstance {
		return new KubernetesSandboxInstance(id, this.config, this.getClient());
	}
