 ...
 */
import { buildGitCloneCommand, pollUntilReady, shellQuote } from '@marimo-hub/compute-commons';
...
export class E2bCompute implements SandboxProvider {
	...
	create(id: SandboxId): SandboxInstance {
		return new E2bSandboxInstance(id, this.config, this.getClient());
	}
...
export function createE2bClient(
	...
	const wrap = (sbx: any): E2bSandboxHandle => ({
		...
		files: {
			write: async (path, content) => void (await sbx.files.write(path, content)),
