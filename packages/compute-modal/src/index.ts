...
class ModalSandboxInstance implements SandboxInstance {
	...
	async writeFiles(files: readonly SandboxFileWrite[]): Promise<void> {
		...
		// The REST write creates no parents, so mkdir first — once for every parent
		// (deduped), and BEFORE any write, since a staged temp file lands in the same
		// directory as its destination.
		const dirs = new Set(files.map((f) => f.path.slice(0, f.path.lastIndexOf('/')) || '/'));
		const mk = await this.exec(`mkdir -p ${[...dirs].map(shellQuote).join(' ')}`);
		if (!mk.success) throw new Error(`writeFiles mkdir failed: ${mk.stderr || mk.stdout}`);

		await mapWithConcurrency(files, WRITE_CONCURRENCY, async (f) => {
			if (typeof f.content === 'string') {
				await this.modal(`/v1/sandboxes/${this.id}/files/write`, {
					path: f.path,
					content: f.content,
				});
				return;
			}
			// `content` is a JSON body field, and JSON.stringify turns a Uint8Array into
			// an index map (`{"0":255,…}`) — silently writing garbage rather than failing.
			// So bytes go over the text channel base64-armored and get decoded in-sandbox.
			const tmp = `${f.path}.b64.tmp`;
			await this.modal(`/v1/sandboxes/${this.id}/files/write`, {
				path: tmp,
				content: base64Encode(f.content),
			});
			const res = await this.exec(
				`base64 -d ${shellQuote(tmp)} > ${shellQuote(f.path)} && rm -f ${shellQuote(tmp)}`,
			);
			if (!res.success) {
				throw new Error(`writeFile ${f.path} failed: ${res.stderr || res.stdout}`);
			}
		});
	}
...
export class ModalCompute implements SandboxProvider {
	...
	create(id: SandboxId, options?: CreateSandboxOptions): SandboxInstance {
		// NOTE: the Modal facade is id-addressed and never sends `image` in a request
		// today, so the override only takes effect if a create call grows one.
		const config = options?.image ? { ...this.config, image: options.image } : this.config;
		return new ModalSandboxInstance(id, config);
	}
