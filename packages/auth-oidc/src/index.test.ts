...
describe('OIDC routes', () => {
	it('redirects login with PKCE, nonce, state, and a signed transaction cookie', async () => {
		const { routes } = makeOidc({ allowedEmailDomains: ['Marimo.IO'] });
		...
		expect(location.searchParams.get('hd')).toBe('marimo.io');
	...
	it('does not include an hd hint when multiple domains are allowed', async () => {
		const { routes } = makeOidc({ allowedEmailDomains: ['marimo.io', 'example.com'] });
