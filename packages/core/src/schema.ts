...
// the list. `member_ids` is a server-side filtering aid, not part of the public
// contract, so it is stripped too. The bytes stay in the persisted snapshot —
// the GC sweep and the list filter read them.
export type PublicProjectEntry = Omit<SnapshotProjectEntry, 'notebooks' | 'member_ids'>;
...
export function toPublicProjectEntry(entry: SnapshotProjectEntry): PublicProjectEntry {
	const { notebooks: _notebooks, member_ids: _member_ids, ...rest } = entry;
	return rest;
}
