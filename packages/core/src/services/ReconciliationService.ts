 ...
 */
const DEFAULT_ORPHAN_GRACE_MS = 15 * 60 * 1000; // 15 minutes
	...
	async reconcile(opts?: { orphanGraceMs?: number }): Promise<ReconcileResult> {
		...
		for (const session of sessions) {
			...
			if (isTerminal && activeIds.has(sandboxId)) {
				...
				// kernel URL is dead; mark the record terminated so it stops being served
				// and gets reaped on schedule.
				try {
					await this.sessions.terminate(session.session_id);
					markedDead++;
				} catch {
