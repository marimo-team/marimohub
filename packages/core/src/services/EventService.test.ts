...
describe('EventService', () => {
	...
	describe('append', () => {
		...
		it('stores one object per event under the day prefix', async () => {
			await events.append({ event: 'a', actor: 'x' });
			await events.append({ event: 'b', actor: 'y' });
		...
		it('returns events in append order', async () => {
			await events.append({ event: 'first', actor: 'a' });
			await events.append({ event: 'second', actor: 'b' });
	...
	describe('getEvents', () => {
		...
		it('parses all events for a day', async () => {
			await events.append({ event: 'a', actor: 'x' });
			await events.append({ event: 'b', actor: 'y' });
			await events.append({ event: 'c', actor: 'z' });
	...
	describe('daily rollover', () => {
		it('writes to separate files for different days', async () => {
			...
			await events.append({ event: 'day1', actor: 'a' });

			vi.setSystemTime(new Date('2025-03-06T00:01:00.000Z'));
			await events.append({ event: 'day2', actor: 'b' });
