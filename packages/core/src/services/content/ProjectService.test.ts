...
describe('ProjectService', () => {
	...
	describe('listProjects visibility filter (MARIMOHUB_DEFAULT_ROLE=none)', () => {
		...
		it('returns all projects when a default role is set', async () => {
			await projects.createProject({ name: 'A', description: 'a' }, ACTOR);
			const list = await projects.listProjects({ userId: STRANGER, defaultRole: 'viewer' });
		...
		it('hides projects a caller does not own or belong to when defaultRole is null', async () => {
			...
			expect(await projects.listProjects({ userId: ACTOR, defaultRole: null })).toHaveLength(1);
			expect(await projects.listProjects({ userId: STRANGER, defaultRole: null })).toEqual([]);
		});
		...
		it('shows a project once the caller is added as a member', async () => {
			...
			await projects.addMember(p.id, STRANGER, 'viewer', ACTOR);
			const list = await projects.listProjects({ userId: STRANGER, defaultRole: null });
