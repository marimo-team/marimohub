...
export class NotebookService {
	...
	async duplicateNotebook(
		...
		const { meta, readme } = await this.getNotebook(projectId, notebookId);
		const code = await this.getNotebookContent(projectId, notebookId);
		const depsObj = await this.bucket.get(paths.project(projectId).notebook(notebookId).deps);
