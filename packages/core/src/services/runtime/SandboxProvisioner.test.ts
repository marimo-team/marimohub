...
describe('SandboxProvisioner', () => {
	...
	describe('provision', () => {
		...
		it('injects sessionEnv (files + env) BEFORE starting the kernel', async () => {
			...
			expect(calls.sequence).toEqual(['writeFile', 'setEnvVars', 'startProcess']);
		});
		...
		it('fallback path: mountBucket throws -> usedFallback true, restoreWorkspace copies the workspace in', async () => {
			...
			// Fallback restored both workspace files into the mount path (base64-decoded
			// via exec, so the binary-safe restore round-trips bytes).
			const restored = calls.exec.filter((c) => c.includes('base64 -d'));
			expect(restored.some((c) => c.includes(`${MOUNT_PATH}/notebook.py`))).toBe(true);
			expect(restored.some((c) => c.includes(`${MOUNT_PATH}/pyproject.toml`))).toBe(true);
