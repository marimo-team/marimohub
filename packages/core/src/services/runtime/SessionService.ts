...
export class SessionService {
	 ...
	 * partition plan 029 introduced so the interactive reads (`listActiveByProject`,
	 ...
	 *
	 * Scoped to the project's own prefix (`_system/sessions/{pid}/`, plan 029), so
	...
	async findReusable(
		...
		const now = Date.now();
		// Scoped to the project prefix (plan 029); filter to the notebook in memory.
		const sessions = await this.scanProject(projectId, (session) => session);
		return sessions
			.filter(
				(s) =>
