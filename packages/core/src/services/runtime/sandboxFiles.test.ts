...
describe('restoreWorkspace', () => {
	it('mkdir -p the working dir, then restores every workspace key into the sandbox', async () => {
		const { nb } = nbCtx();
		...
		expect(calls.exec.some((c) => c.startsWith(`mkdir -p `) && c.includes(MOUNT))).toBe(true);
		// Nested keys get their parent dir created first (SDK writeFile won't make parents).
		expect(calls.exec).toContain(`mkdir -p '${MOUNT}/data'`);
	...
	it('streams the payload through writeFile, never inlining it into an exec argv (ARG_MAX-safe)', async () => {
		const { nb } = nbCtx();
		const bucket = new MemoryBucket();
		// A blob whose base64 (~273 KiB) would blow past a single shell argument if
		// inlined; it must instead flow through writeFile to a temp file.
		...
		// No exec command carries the base64 payload — restore decodes from a temp file.
		expect(calls.exec.every((c) => c.length < 4096)).toBe(true);
		expect(calls.exec.some((c) => c.startsWith('base64 -d '))).toBe(true);
	});
