 ...
 */
const DEFAULT_TTL_MS = 10 * 60 * 1000; // 10 minutes — comfortably > one sweep
...
export class MaintenanceLock {
	 ...
	 */
	async acquire(holder: string, ttlMs = DEFAULT_TTL_MS): Promise<boolean> {
		const now = Date.now();
		...
		const existing = await this.bucket.get(paths.maintenanceLock);
		if (!existing) {
		...
		try {
			await this.bucket.put(paths.maintenanceLock, body, { onlyIfEtagMatches: existing.etag });
			return true;
		} catch (err) {
	...
	async release(holder: string): Promise<void> {
		...
		await this.bucket.delete(paths.maintenanceLock).catch(() => {});
	}

	private async tryCreate(body: string): Promise<boolean> {
		try {
			await this.bucket.put(paths.maintenanceLock, body, { onlyIfNotExists: true });
			return true;
		} catch (err) {
