...
const DEFAULT_SNAPSHOT_RETENTION_MS = 90 * DAY_MS;
const DEFAULT_EVENT_RETENTION_MS = 90 * DAY_MS;
/**
...
export class MaintenanceService {
	...
	async expireSnapshots(opts?: ExpireSnapshotsOptions): Promise<number> {
		...
		const objects = await this.listAll(paths.snapshotsPrefix);
	...
	async pruneEvents(opts?: PruneEventsOptions): Promise<number> {
		...
		for (const dayPrefix of delimitedPrefixes) {
			...
			const dayObjects = await this.listAll(dayPrefix);
	...
	/** List every object under a prefix, following the list cursor. */
	private async listAll(prefix: string) {
		const all: Awaited<ReturnType<Bucket['list']>>['objects'] = [];
