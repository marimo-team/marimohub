...
/**
 * The tags/status text for a notebook row: its tags if any, otherwise a
 ...
 */
function notebookLabel(nb: NotebookEntry): string | null {
	if (nb.tags.length > 0) return nb.tags.join(', ');
	return nb.status === 'active' ? null : nb.status;
}
...
const notebookNameSchema = z.object({ name: requiredText('Notebook name') });
...
export function Project() {
	...
	useSeedOnOpen(createNotebookForm, uploadModal.isOpen, { name: '' });
	...
	return (
		<PageContainer>
			...
			>
				...
				<h1 className="truncate text-xl font-semibold tracking-tight" title={project.name}>
					{project.name}
				</h1>
				...
				<IconButton label="Project secrets" tooltip="Project secrets" onPress={secretsModal.open}>
			...
			{filteredNotebooks.length === 0 ? (
				searchQuery ? (
					<EmptyState message={`No notebooks matching "${searchQuery}"`} />
				) : (
					<EmptyState
						message="No notebooks yet"
						action={
							<Button variant="ghost" onPress={uploadModal.open}>
						...
						return (
							...
							>
								...
								<div className="flex shrink-0 items-center gap-3">
									...
									/>
									{label && (
										<span className="text-xs text-muted-foreground max-md:hidden">{label}</span>
									)}
