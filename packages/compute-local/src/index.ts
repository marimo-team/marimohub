...
class LocalSandboxInstance implements SandboxInstance {
	...
	private rewriteCmd(cmd: string): string {
		return cmd.replaceAll(WORKSPACE, path.join(this.root, WORKSPACE));
	}

	/**
	 * Adjust a `uv run … marimo …` command for local execution:
	 *  - `--with marimo` so marimo is available even if the notebook's
	 *    pyproject.toml doesn't declare it (installed into an ephemeral env).
	 *  - `--host <bindHost>` when binding to a non-default interface (Docker:
	...
	private prepareMarimoCmd(cmd: string): string {
		...
		if (this.bindHost !== '127.0.0.1' && out.includes('marimo') && !out.includes('--host')) {
			out = `${out} --host ${this.bindHost}`;
		}
		return out;
	}
	...
	async gitCheckout(repo: string, options?: GitCheckoutOptions): Promise<void> {
		...
		const res = await this.exec(`git clone ${branch} ${repo} ${target}`.trim());
