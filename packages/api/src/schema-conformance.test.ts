...
describe('schema conformance: api response shapes vs core public shapes', () => {
	...
	// defeats the page cursor) and the denormalized `member_ids` (a server-side
	// visibility-filter aid, not part of the public contract). `notebook_count`
	// stays for the summary; clients page `GET /projects/{pid}/notebooks`.
	it('SnapshotProjectEntry omits `notebooks` and `member_ids` from the core entry', () => {
		const coreKeys = shapeKeys(CoreSnapshotProjectEntrySchema);
		const apiKeys = shapeKeys(SnapshotProjectEntrySchema);
		const internal = ['notebooks', 'member_ids'];
