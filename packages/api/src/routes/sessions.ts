		...
		await saga(observer)
			...
			.step('sandbox_provision', {
				do: async () => {
					...
					// Project secrets: resolve every configured entry to plaintext and inject
					// it. Unlike WIF/AI (best-effort), this FAILS CLOSED — a key the author
					// configured is load-bearing, so a resolve failure aborts session create
					// rather than starting a kernel that silently lacks it. Never injected into
					// an ephemeral (viewer) sandbox, and never logged. Merged first so a
					// hub-managed system/WIF var wins a name collision.
					if (deps.secrets && !ephemeral) {
						try {
							const secretVars = await deps.secrets.resolve(pid);
							observer.tag('secrets_injected_count', Object.keys(secretVars).length);
							// Secrets are the base; the existing (system/WIF/AI) vars win a
							// name collision, so a user secret can never shadow them.
							sessionEnv = {
								files: sessionEnv?.files ?? [],
								vars: { ...secretVars, ...sessionEnv?.vars },
							};
						} catch (err) {
