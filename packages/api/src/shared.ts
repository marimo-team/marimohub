...
import type { Project, ProjectService, Role, Session, UserId } from '@marimo-hub/core';
...
export async function assertProjectRole(
	...
	requireRole(project, userId, min, defaultRole);
	return project;
}
...
export async function assertSessionControl(
	...
	if (
		...
		canAct(project, userId, 'viewer', defaultRole)
	) {
		return;
	}
	requireRole(project, userId, 'editor', defaultRole);
}
...
export async function loadVisibleProject(
	...
	const project = await projects.getProject(pid);
	if (!canAct(project, userId, 'viewer', defaultRole)) {
...
export async function assertProjectVisible(
	...
	await loadVisibleProject(projects, pid, userId, defaultRole);
}
