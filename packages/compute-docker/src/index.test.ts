...
describe('DockerCompute', () => {
	...
	it('writeFile throws when mkdir fails', async () => {
		...
		await expect(sb.writeFile('/root/notebook.py', 'x=1')).rejects.toThrow(
			/writeFile mkdir failed.*permission denied/,
		);
	});

	it('writeFile throws when the stdin write fails', async () => {
		const { runner } = fakeRunner((args) => {
			const base = defaultHandler(args);
			if (base) return base;
		...
		await expect(sb.writeFile('/workspace/notebook.py', 'x=1')).rejects.toThrow(
			/writeFile failed.*disk full/,
		);
	});
