...
describe('service account configuration', () => {
	...
	it.each([
		...
		['trailing newline in ID', { ...account, id: 'deploy\n' }, '0.id'],
	...
	it.each([
		...
		['trailing newline in credential ID', { ...credential, id: 'initial\n' }],
		['trailing newline in hash', { ...credential, sha256: `${credential.sha256}\n` }],
