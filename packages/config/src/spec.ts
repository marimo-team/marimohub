...
export const CONFIG_SPEC: ConfigGroup[] = [
				...
				vars: [
					{
						...
						name: 'Sandbox image',
						description:
							'Container image with marimo + uv + python. Required by the `modal` backend; recommended for `coreweave`.',
					...
					},
					{
						...
						example: 'https://cdn.jsdelivr.net/npm/@marimo-team/...',
					},
				],
			},
			{
			...
			},
			{
				...
				vars: [
					...
					},
					{
						...
						name: 'E2B template',
						description:
							'E2B template id with marimo + uv + python. Falls back to `MARIMOHUB_COMPUTE_IMAGE`.',
						example: 'marimo',
					},
					{
		...
		backends: [
			{
				...
				vars: [
					...
					},
					{
						...
						description:
							'Comma-separated email-domain allowlist (requires a verified email). Required so an internal deployment cannot silently admit every account the IdP authenticates; set `*` to explicitly allow all domains. A single domain is also sent to Google as the `hd` hint.',
						example: 'marimo.io,example.com',
						required: true,
					},
				],
			},
	...
	},
	{
		...
		description:
			'Optional: let a notebook reach an object-storage bucket with NO long-lived key. The hub becomes an OIDC issuer and, per session, mints a short-lived project-scoped JWT and exchanges it server-side (via the selected broker) for temporary S3 credentials, which it injects into the sandbox — the JWT itself never reaches the sandbox. Deployment-wide capability; each project opts in via its `federation` setting. All-or-nothing on the generic vars: set them to enable, or none to disable. See docs/coreweave-bucket-access.md.',
		backends: [
			{
				...
				vars: [
					{
						...
						name: 'WIF broker',
						description:
							'Which credential broker exchanges the JWT for temporary creds. Required when WIF is enabled (no default, so the federated cloud is always explicit). Currently `coreweave`; add an adapter implementing the core CredentialBroker port for more.',
						example: 'coreweave',
					},
					{
